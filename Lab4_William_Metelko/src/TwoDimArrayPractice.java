...
public class TwoDimArrayPractice extends JFrame
{
   ...
   private JLabel threadSlideText;
   ...
   private static int threadSleepAmnt = 100;
   ...
   public TwoDimArrayPractice( )
   {
      ...
      threadSlideText = new JLabel("Animation Speed: ");
      c.add(threadSlideText);
      threadSlider = new JSlider(0,500,threadSleepAmnt);
      c.add(threadSlider);
	...
	threadSlider.setForeground(Color.black);
	threadSlider.addChangeListener(
		e -> 
		{
			threadSleepAmnt = 500 - threadSlider.getValue();
		}
	);
   ...
   */
   public int findMinimum( int column )
   {
    ...
    // Write your code here:
	   int lowest = intArray[0][column];
	   int lowestIndex = 0;
	   
	   for(int i = 0; i < intArray.length; i++) {
		   System.out.println( intArray.length);
		   if(intArray[i][column] < lowest) {
			   lowest = intArray[i][column];
			   lowestIndex = i;
		   }
	   }
	   animate(lowestIndex, column, lowest);

      return lowest; // replace this line with your return statement
   }
	   ...
	   for(int i = 0; i < intArray.length; i++) {
		   for(int j = 0; j < intArray[i].length; j++) {
			   if(intArray[i][j] == value) {
				   timesFound++;
				   animate(i, j, intArray[i][j]);
			   }
		   }
	   }
	   
     return timesFound; // replace this line with your return statement
   }
